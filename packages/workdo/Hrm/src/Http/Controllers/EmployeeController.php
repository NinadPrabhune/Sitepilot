...
//        var_dump(getActiveWorkSpace());
//        var_dump(getActiveProject());
...
//        $canUse =  PlanCheck('User', Auth::user()->id);
        ...
        Log::info('EmployeeController::store - Debug Info', [
            'user_id' => Auth::id(),
        ...
        if (empty($company_settings)) {
            Log::warning('EmployeeController::store - Company settings are EMPTY! This will cause "not set configuration" error.');
        }
//        if ($canUse == false) {
...
//        }
        ...
        if (Auth::user()->isAbleTo('employee create')) {
            ...
            if (!empty($request->avatar) && isset($path['url'])) {
                $avatarPath = $path['url'];
            }
            ...
            $employee = Employee::create(
                    [
                        ...
                        'avatar' => $avatarPath, // store uploaded file path
                        'organisation_switch' => $request['organisation_switch'],
                        'provident_fund_no' => $request['provident_fund_no'],
            ...
            if (!empty($projectIds)) {
                ...
                // Validate workspace exists in work_spaces table
                $workspaceExists = \App\Models\WorkSpace::where('id', $workspaceId)->exists();
                if (!$workspaceId || !$workspaceExists) {
                    // Fall back to user's current workspace if not valid
                    $workspaceId = $user->workspace_id ?? getActiveWorkSpace();
                }
    ...
    public function show($id) {
        if (Auth::user()->isAbleTo('employee show')) {
            ...
            // Check if user is Admin or company - show without workspace restriction
...
//            dd(getActiveWorkSpace());
        ...
        if (Auth::user()->isAbleTo('employee edit')) {
            ...
            // DEBUG: Log configuration state for edit method
            $workspaceId = getActiveWorkSpace();
            $creator = creatorId();
            $company_settings = getCompanyAllSetting();
            
            Log::info('EmployeeController::edit - Debug Info', [
                'employee_id' => $id,
                'user_id' => Auth::id(),
                'user_type' => Auth::user()->type ?? 'N/A',
                'active_workspace' => $workspaceId,
                'creatorId' => is_object($creator) ? 'response_object' : $creator,
                'company_settings_empty' => empty($company_settings),
            ]);
            
            if (empty($workspaceId)) {
                Log::error('EmployeeController::edit - getActiveWorkSpace() returned NULL! This will cause issues.');
            }
...
//            $employee     = Employee::where('user_id', $id)->where('workspace', getActiveWorkSpace())->first();
            ...
            $employee_document = EmployeeDocument::where('employee_id', $employee->id)->first();

            if ($employee_document) {
               
                // If a document exists, use its workspace
                // Global master data - no workspace filter
            ...
            $selectedProjects = $user->projects()->pluck('project_id')->toArray();
            
             $role = Role::where('created_by', creatorId())->whereNotIn('name', Auth::user()->not_emp_type)->get()->pluck('name', 'id');
            
             $selectedRoleId = $role->search($user->type); // find the id by name
          
//             dd($employee->name);
             
//             dd($selectedRoleId);
            return view('hrm::employee.edit', compact('employee', 'user', 'employeesId', 'branches', 'departments', 'designations', 'document_types', 'customFields', 'location_type', 'selectedProjects','role','selectedRoleId'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function update(Request $request, $id) {
        if (Auth::user()->isAbleTo('employee edit')) {
            ...
            // DEBUG: Log configuration state for update method
            $workspaceId = getActiveWorkSpace();
            $company_settings = getCompanyAllSetting();
            
            Log::info('EmployeeController::update - Debug Info', [
                'employee_id' => $id,
                'user_id' => Auth::id(),
                'active_workspace' => $workspaceId,
                'company_settings_empty' => empty($company_settings),
...
//                   dd($path);
//                    if (Storage::disk('public')->exists($path['url'])) {
//                        dd('in');
//                    }
                // old img delete
...
if ($request->has('document')) {
    foreach ($request->file('document') as $docId => $file) {
        if (!empty($file)) {
            ...
            // Upload file
            $upload = multi_upload_file($file, 'document', $fileNameToStore, 'emp_document');
            if ($upload['flag'] == 1) {
                $url = $upload['url'];
            } else {
            ...
            // Find existing document by employee_id + document_id
            $employee_document = EmployeeDocument::where('employee_id', $employee->id)
...
//                dd('else');
            ...
            // Fill other fields including organisation_switch and provident_fund_no
            $employee->organisation_switch = $request->organisation_switch;
            $employee->provident_fund_no = $request->provident_fund_no;

            $employee->emergency_contact_no = $request->emergency_contact_no;
            $employee->emergency_address = $request->emergency_address;

            $employee->fill($request->except(['avatar']))->save();
            ...
            if (!empty($roles)) { 
                ...
                // Sync roles - remove all existing roles and add the new role
                $user->roles()->detach();
                $user->addRole($roles);
            }
            ...
            if (Auth::user()->isAbleTo('employee manage')) {
                ...
                DB::table('user_projects')->where('user_id', $user->id)->delete();
                ...
                Log::info('Project IDs from request', ['project_ids' => $projectIds]);
                // Optionally set the first selected project as active
                if (!empty($projectIds)) {
                    $firstProjectId = $projectIds[0];
                    
                    Log::info('Project IDs from request', ['firstProjectId' => $firstProjectId]);
                    $workspaceId = getWorkspaceIDFromSiteID($firstProjectId);

                    Log::info('Workspace ID resolved', ['workspace_id' => $workspaceId]);
                    ...
                    if (!$workspaceId || !$workspaceExists) {
                        // Fall back to user's current workspace if not valid
                        $workspaceId = $user->workspace_id ?? getActiveWorkSpace();
                        Log::info('Workspace ID fallback used', ['workspace_id' => $workspaceId]);
                    }
...
//    }
    ...
    public function getdDesignation(Request $request) {
        // Global master data - filter by department_id only if provided
        if ($request->department_id == 0 || empty($request->department_id)) {
            $designations = Designation::all()->pluck('name', 'id')->toArray();
        } else {
            $designations = Designation::where('department_id', $request->department_id)->pluck('name', 'id')->toArray();
        }
            ...
            foreach ($file_data as $key => $row) {
                ...
                if ($employees->isEmpty()) {
                    ...
                    try {
                        $user = User::create(
                                [
                                    'name' => $this->toCamelCase($row[$request->name]),
