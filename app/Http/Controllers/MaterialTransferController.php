    ...
    public function store(Request $request) {
        ...
        try {
            ...
            try {
                ...
                $availableStock = getCurrentStockBySiteId($siteId, null, null, null, null, null)
                                    ->firstWhere('material_id', $materialId)?->total_qty ?? 0;
                ...
                if ($item['quantity'] <= 0) {
                    throw new \Exception("Invalid quantity for material ID {$materialId}");
                }
                ...
                return redirect()->back()->with('success', 'Material transfer created successfully.');

//                return redirect()->route('material-transfer.index')->with('success', 'Material transfer created successfully.');
...
//                    $currentQty = $materialTransfer->items->where('material_id', $materialId)->sum('quantity');
    ...
    public function getStockBySiteMaterialTransferEdit(Request $request) {
        try {
            ...
            // Get full stock list
            $stock = getCurrentStockBySiteId($siteId, $excludeConsumptionId = null, $materialTransferId, null, null, null);
