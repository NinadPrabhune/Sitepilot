                ...
                <div class="card">
                    <div class="card-body pt-1">
                        <div class="">
                            ...
                            {{ Form::text('name', null, ['class' => 'form-control', 'required' => 'required', 'placeholder' => __('Enter Role Name'), 'id' => 'role_name', 'onblur' => 'checkAutoSelectPermissions()']) }}
                            @error('name')
                ...
                @foreach ($modules as $module)
                    <div class="card tab-pane h-100 mb-0  fade {{ $loop->index == 0 ? 'show active' : '' }}"
                                            ...
                                            <table class="table table-striped table-roles mb-0 mt-3">
                                                ...
                                                <tbody>
                                                    @php
                                                        ...
                                                        $getSystemActiveModule=getSystemActiveModule();
                                                        
                                                        
                                                        $module_list = array_intersect($module_list, $getSystemActiveModule);
                                                                    ...
                                                                    @foreach ($permissions as $key => $prermission)
                                                                        ...
                                                                        @if ($list == $check)
                                                                            <div class="col-xl-3 col-sm-6 form-check mb-2">
                                                                                {{ Form::checkbox('permissions[]', $prermission->id, false, ['class' => 'form-check-input pointer checkbox-' . strtolower($module) . ' auto-select-admin-company', 'id' => 'permission_' . $key . '_' . $prermission->id, 'data-role-check' => 'true']) }}
                        ...
                        <div class="card-footer p-3 d-flex justify-content-end gap-2">
                            <input type="button" onclick="location.href = '{{ route('roles.index') }}';" value="{{ __('Cancel') }}"
...
<script>
    ...
    function checkAutoSelectPermissions() {
        var roleName = $('#role_name').val().toLowerCase();
        if (roleName === 'admin' || roleName === 'company') {
            $('.auto-select-admin-company').prop('checked', true);
        } else {
            $('.auto-select-admin-company').prop('checked', false);
        }
    }
</script>
