...
class MachineryDataTable extends DataTable {
    ...
    {
        return \App\Models\Machinery::class;
    }
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable {
        ...
        $dataTable = (new EloquentDataTable($query))
                ->addIndexColumn()
                ->addColumn('checkbox', function (Machinery $model) {
                    return '<input type="checkbox" class="' . $this->getCheckboxClass() . ' form-check-input" value="' . $model->id . '">';
                })
                ->editColumn('site_id', function (Machinery $model) {
                    return optional($model->site)->name ?? '';
                })
                ->editColumn('category_id', function (Machinery $model) {
                    return optional($model->category)->name ?? '';
                })
                ->editColumn('operational_status', function (Machinery $model) {
        ...
        if (\Laratrust::hasPermission('machinery show') || \Laratrust::hasPermission('machinery edit') || \Laratrust::hasPermission('machinery delete')) {
            $dataTable->addColumn('action', function (Machinery $machinery) {
                return view('machineries.action', compact('machinery'));
            });
            $rowColumn[] = 'action';
        }
     ...
     */
    public function query(Machinery $model): QueryBuilder {

        $machineryQuery = $model->with(['category', 'site'])->where('workspace_id', getActiveWorkSpace())->where('site_id', getActiveProject())->orderBy('id', 'desc');

        // Handle selected_ids from export request using trait method
        $this->handleSelectedIdsFilter($machineryQuery);

        return $machineryQuery;
    }
