{{-- DPR Edit Form - Safe Version with Separate JS --}}
{{-- This version uses data attributes instead of inline scripts --}}
...
<script src="{{ asset('js/dpr-ajax-handler.js') }}"></script>

{{-- Trigger initialization with data --}}
<script>
// Safe initialization without inline data processing
$(document).ready(function() {
    try {
        // Extract data from the hidden div
        const dprData = $('#dpr-data').data();
        
        // Trigger custom event with the data
        $(document).trigger('dpr:modalLoaded', [dprData]);
        
        console.log('DPR Modal initialized safely');
    } catch (error) {
        console.error('DPR initialization error:', error);
    }
});
</script>
