                ...
                <td>
                    @if($attendance->clock_in_image)
                        <div class="d-flex align-items-center gap-2">
                            <img src="{{ asset($attendance->clock_in_image) }}" alt="Clock In" style="max-width: 150px; cursor: pointer;" onclick="openImagePreview({images: ['{{ asset($attendance->clock_in_image) }}']})">
                            @include('components.image-preview-button', ['src' => asset($attendance->clock_in_image), 'text' => 'Preview', 'class' => 'btn btn-sm btn-info'])
                        </div>
                    @else
            ...
            <tr>
                ...
                <td>
                    @if($attendance->clock_out_image)
                        <div class="d-flex align-items-center gap-2">
                            <img src="{{ asset($attendance->clock_out_image) }}" alt="Clock Out" style="max-width: 150px; cursor: pointer;" onclick="openImagePreview({images: ['{{ asset($attendance->clock_out_image) }}']})">
                            @include('components.image-preview-button', ['src' => asset($attendance->clock_out_image), 'text' => 'Preview', 'class' => 'btn btn-sm btn-info'])
                        </div>
                    @else
                        <span class="text-muted">No image</span>
                    @endif
