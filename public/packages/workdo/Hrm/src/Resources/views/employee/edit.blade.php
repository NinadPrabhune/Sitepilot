...
<div class="row">
    ...
    <div class="card-body employee-detail-create-body p-0">
        @php
            ...
            $employeedoc = !empty($employee->documents)
                ? $employee->documents()->pluck('document_value', 'document_id')->toArray()
                : [];
    ...
    @php
        ...
        $docUrl  = $docPath ? get_file($docPath) : asset('images/material/No_Image_Available.jpeg');
        $ext     = $docPath ? pathinfo($docPath, PATHINFO_EXTENSION) : null;
    @endphp

    <!-- Always render an <img> placeholder -->
    <img id="{{ 'blah' . $key }}"
         src="{{ in_array(strtolower($ext), ['jpeg','jpg','png','gif','svg']) ? $docUrl : asset('images/material/No_Image_Available.jpeg') }}"
         alt="your image"
         width="120" height="100"
         class="rounded mb-2"
         style="{{ in_array(strtolower($ext), ['jpeg','jpg','png','gif','svg']) ? '' : '' }}" />

    <!-- Show link for non-image files -->
    @if($docPath && !in_array(strtolower($ext), ['jpeg','jpg','png','gif','svg']))
        <a href="{{ $docUrl }}" target="_blank" class="btn btn-sm btn-primary">
            View File
        </a>
    @endif
</div>
