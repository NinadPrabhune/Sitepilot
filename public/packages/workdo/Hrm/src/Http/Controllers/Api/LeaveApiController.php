...
class LeaveApiController extends Controller {
    ...
    public function index(Request $request) {
        try {
            ...
            $leaves = Leave::with('leaveType')->where('user_id', '=', $request->user_id)
//                    ->where('workspace', $request->workspace_id)
                    ...
                    ->map(function ($leave) {
                        return [
                            ...
                            "leave_type" => $leave->leaveType, // include relation
    ...
    public function show($id) {
        try {
            ...
            return response()->json(['status' => 1, 'data' => $leave], 200);
        } catch (\Exception $e) {
    ...
    public function update(Request $request, $id) {
        try {
            ...
            $leave->leave_reason = $request->leave_reason;
            $leave->remark = $request->remark;
            $leave->status = $request->status ?? $leave->status; // optional status update
            $leave->save();

            return response()->json(['status' => 1, 'data' => $leave, 'message' => 'Leave successfully updated.'], 200);
        } catch (\Exception $e) {
            return response()->json(['status' => 0, 'message' => 'Something went wrong!!!'], 500);
        }
    }
