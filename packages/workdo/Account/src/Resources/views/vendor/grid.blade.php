                ...
                <div class="card-header border border-bottom p-3 h-100">
                    <div class="user-img-wrp d-flex align-items-center">
                        <div class="user-image rounded border-2 border border-primary">
                            <a href="{{ check_file($Vendor->avatar) ? get_file($Vendor->avatar) : 'uploads/users-avatar/avatar.png' }}" target="_blank">
                        ...
                        <div class="user-content">
                            @permission('vendor show')
                                @if($Vendor->is_disable == 1 && !empty($Vendor['id']))
                                    <a href="{{ route('vendors.show', \Crypt::encrypt($Vendor['id'])) }}">
                                        <h4 class="mb-2 text-break">{{ !empty($Vendor->name) ? $Vendor->name : '' }}</h4>
                                    </a>
                                @else
                                    <h4 class="mb-2 text-break">{{ !empty($Vendor->name) ? $Vendor->name : '' }}</h4>
                                @endif
                            ...
                            <span class="text-dark text-md">{{ $Vendor->email }}</span>
                    </div>
                    </div>
                </div>
                <div class="card-body text-center p-3">
                    <div class="bottom-icons d-flex flex-wrap align-items-center justify-content-between">
                        <div class="edit-btn-wrp d-flex flex-wrap align-items-center">
                            ...
                            @if (Laratrust::hasPermission('vendor show') || Laratrust::hasPermission('vendor edit') || Laratrust::hasPermission('vendor delete'))
                                    @if (!empty($Vendor['id']))
                                        ...
                                        @permission('vendor edit')
                                            <a href="#!" data-url="{{ route('vendors.edit', $Vendor['id']) }}"
                                            data-ajax-popup="true" data-size="lg" data-title="{{ __('Edit') }}"
                                        ...
                                        @permission('vendor delete')
                                            ...
                                            data-bs-original-title="{{ __('Delete') }}" data-bs-toggle="tooltip" data-bs-placement="top"
                            ...
                            data-bs-original-title="{{ __('Vendor Is Disable') }}" data-bs-toggle="tooltip">
                        ...
                        @if (!empty($Vendor['id']))
                            <span class="badge bg-primary p-2 px-3">
                                @permission('vendor show')
                                @if($Vendor->is_disable == 1)
                                    <a href="{{ route('vendors.show', \Crypt::encrypt($Vendor['id'])) }}" class="text-white">
    ...
    @permission('vendor create')
        <div class="col-xxl-3 col-xl-4 col-md-6">
            <a href="#" class="btn-addnew-project border-primary p-4" data-ajax-popup="true" data-size="lg"
                data-title="{{ __('Create Vendor') }}" data-url="{{ route('vendors.create') }}">
                <div class="bg-primary proj-add-icon">
                    <i class="ti ti-plus my-2"></i>
                </div>
                <h6 class="my-2 text-center">{{ __('New Vendor') }}</h6>
                <p class="text-muted text-center mb-0">{{ __('Click here to Create New Vendor') }}</p>
            </a>
        </div>
    @endpermission
    @endauth
</div>
    {!! $vendors->links('vendor.pagination.global-pagination') !!}
@endsection
