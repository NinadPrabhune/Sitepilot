...
class SettingsController extends Controller
{
    ...
    public function store(Request $request){
        if(Auth::user()->isAbleTo('setting manage'))
        {
            ...
            // Invalidate numbering settings cache with version bump
            // This ensures all cached settings are invalidated immediately
            $numberingModules = ['po', 'indent', 'grn', 'invoice', 'payment'];
            foreach ($numberingModules as $module) {
                // Increment cache version to invalidate all versioned cache keys
                $currentVersion = Cache::get("numbering_settings_version_{$module}", 1);
                Cache::put("numbering_settings_version_{$module}", $currentVersion + 1);
            }
