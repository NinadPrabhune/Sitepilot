...
/**
 * InvoiceDataTable - DataTable with selectable export functionality
 * 
...
class InvoiceDataTable extends DataTable
{
    ...
    {
        $rowColumn = ['invoice_id', 'issue_date', 'due_date', 'total_amount', 'due_amount', 'status', 'action'];

        $dataTable = (new EloquentDataTable($query))
            ->addIndexColumn()
            ->addColumn('checkbox', function (Invoice $invoice) {
                return '<input type="checkbox" class="' . $this->getCheckboxClass() . ' form-check-input" value="' . $invoice->id . '">';
            })
            ...
            ->editColumn('due_date', function (Invoice $invoice) {
                if ($invoice->due_date < date('Y-m-d')){
                    return '<p class="text-danger">'. company_date_formate($invoice->due_date) .'</p>';
                }
                else{
