    ...
    public function assignPlan($plan_id = null,$duration = null,$modules = null,$counter = null,$user_id = null)
    {
        ...
        if($plan)
        {
            ...
            if(!empty($modules))
            {
                ...
                $client_role = Role::where('name','client')->where('created_by',$user->id)->where('status',1)->first();
                $staff_role = Role::where('name','staff')->where('created_by',$user->id)->first();
                $vendor_role = Role::where('name','vendor')->where('created_by',$user->id)->where('status',1)->first();
                $hr_role = Role::where('name','hr')->where('created_by',$user->id)->where('status',1)->first();

                $project_manager_role = Role::where('name','Site / Project Manager')->where('created_by',$user->id)->first();
                $account_manager_role = Role::where('name','Account Manager')->where('created_by',$user->id)->first();
               ...
               if(!empty($project_manager_role))
               {
                   event(new GivePermissionToRole($project_manager_role->id,'Site / Project Manager',$modules));
               }
               if(!empty($account_manager_role))
               {
                   event(new GivePermissionToRole($account_manager_role->id,'Account Manager',$modules));
               }
