...
class DashboardController extends Controller {
    ...
    public function index() {
        if (Auth::user()->isAbleTo('taskly dashboard manage')) {
            ...
            if($currentProject){            
                $project = Project::find($currentProject);
                $totalMembers = (int) $project->users->count() + (int) $project->clients->count() + (int) $project->venders->count();
            }
            $totalMaterials=0;
            // Call your function
            $materials = getCurrentStockBySiteId($currentProject);

            // ✅ Get count of materials with stock > 0
            $totalMaterials = $materials->filter(function ($item) {
                return $item->total_qty > 0;
            })->count();

            $uniqueSupplierCount1 = DB::table('purchase_invoices')
                                ->where('site_id', $currentProject)
                                ->distinct('supplier_id')
                                ->count('supplier_id');
            
            $uniqueSupplierCount2 = DB::table('man_power_masters')
                                ->where('site_id', $currentProject)
                                ->distinct('supplier_id')
                                ->count('supplier_id');
            
            $uniqueSupplierCount=$uniqueSupplierCount1+$uniqueSupplierCount2;


            
            return view('taskly::index', compact('uniqueSupplierCount','totalMaterials','currentWorkspace', 'totalProject', 'totalBugs', 'totalTask', 'totalMembers', 'arrProcessLabel', 'arrProcessPer', 'arrProcessClass', 'completeTask', 'tasks', 'chartData', 'totaltasks'));
        } else {
