@permission('material-category show')
<!--    <div class="action-btn me-2">
        <a href="{{ route('units.show', $MaterialCategory->id) }}"
            class="mx-3 btn btn-sm align-items-center bg-warning"
            data-bs-toggle="tooltip" title="{{ __('View') }}"><i class="ti ti-eye text-white"></i></a>
    </div>-->
@endpermission
@permission('material-category edit')
    @if(!in_array($MaterialCategory->id, [2, 3]))
    <div class="action-btn me-2">
        <a class="mx-3 btn btn-sm  align-items-center bg-info" data-url="{{ route('material-categories.edit', $MaterialCategory->id) }}"
            data-ajax-popup="true" data-size="lg " data-bs-toggle="tooltip" title="{{ __('Edit') }}"
            data-title="{{ __('Edit Material Category') }}">
            <i class="ti ti-pencil text-white"></i>
        </a>
    </div>
    @endif
@endpermission
@permission('material-category delete')
    @if(!in_array($MaterialCategory->id, [2, 3]))
    <div class="action-btn">
        {!! Form::open([
            'method' => 'DELETE',
            'route' => ['material-categories.destroy', $MaterialCategory->id],
            'id' => 'delete-form-' . $MaterialCategory->id,
        ]) !!}
        <a href="#" class="mx-3 btn btn-sm  align-items-center bs-pass-para show_confirm bg-danger" data-bs-toggle="tooltip"
            title="{{ __('Delete') }}"><i class="ti ti-trash text-white"></i></a>
        {!! Form::close() !!}
    </div>
    @endif
@endpermission
