@if ($employees->is_disable == 1)
    <span>
        ...
        @if (\Auth::user()->id != $employees->user_id )
    ...
    <div class="action-btn me-2 ms-2">
        <a class="mx-3 btn btn-sm align-items-center bg-success"
           data-url="{{ route('general_transfer.create', ['transfer_type' => 'employee', 'employee_id' => $employees->ID]) }}"
           data-ajax-popup="true"
           data-size="xl"
           data-bs-toggle="tooltip"
           title="{{ __('Create Employee Transfer') }}"
           data-title="{{ __('New Employee Transfer') }}">
