<div class="modal-body">
    ...
    <div class="row">
        <div class="col-md-6">
            <x-form-input
                ...
                label="Issue To Type"
                type="select"
                :options="['user' => __('User (Employee)'), 'supplier' => __('Supplier (Subcontractor)')]"
                :required="true"
                id="issue_to_type"
            />
        </div>
        <div class="col-md-6">
            <div class="form-group mb-3">
                <label for="issue_to_id" class="form-label">
                    {{ __('Issue To') }}
    ...
    <div class="table-responsive">
        <button type="button" class="btn btn-sm btn-primary float-end" id="add-row">
    ...
    $(document).ready(function() {
            ...
            if (materialId) {
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        ...
                        if (xhr.status === 419) {
                            showValidationError('{{ __("Session expired. Please refresh the page.") }}');
                        } else if (xhr.status === 500) {
                            showValidationError('{{ __("Server error. Please try again.") }}');
                        }
                    }
                });
            }
        ...
        $(document).on('input change', '.quantity-input', function() {
            ...
            if (maxStock === 0 && quantity > 0) {
                ...
                $feedback.html('<div class="text-danger small mt-1">{{ __("No stock available for this material") }}</div>');
            } else if (maxStock > 0 && quantity > maxStock) {
                // Cap the quantity to max available stock
                $input.val(maxStock);
                $input.addClass('is-invalid');
                $availableStock.removeClass('bg-success bg-secondary').addClass('bg-danger');
                $feedback.html('<div class="text-danger small mt-1">{{ __("Quantity cannot exceed available stock. Maximum: ") }}' + maxStock + '</div>');
            } else if (quantity > 0) {
