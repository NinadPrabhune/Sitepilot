...
class Kernel extends ConsoleKernel
{
    ...
    public function call($command, array $parameters = [], $outputBuffer = null)
    {
        ...
        // Multi-layer protection checks (config-level first, then database-level)
        ...
        // Config-level protection: isDestructive() already validates environment protection & patterns.
        // If it returned true, the environment IS protected and the command IS destructive.
        // DB-level locks (areCommandsLocked) serve as an additional runtime override but are
        // NOT required — config-level protection is the primary gate.
        if ($isDestructive) {
            $shouldBlock = true;
            $blockReason = "Destructive commands are blocked in {$currentEnvironment} environment";
