...
//            $workspace_users  = User::where('created_by', creatorId())->emp()->where('workspace_id', getActiveWorkSpace())->orWhere('id', Auth::user()->id)->get();
            ...
            $workspace_users  = User::where('created_by', creatorId())->emp()->orWhere('id', Auth::user()->id)->get();
    ...
    public function store(Request $request)
    {
        ...
        if (Auth::user()->isAbleTo('project create')) {
            ...
            // Always ensure creator has access
...
//                dd($registerUsers)
                ...
                if ($registerUsers) {
                    ...
                    Log::error('Project user assignment failed: User not found', [
                        'email' => $email,
...
//        var_dump("ninad");
        if (\Auth::user()->isAbleTo('project show')) {
            ...
            $isProjectUser = $project->users->contains('id', $user->id);

            if ($user->type !== 'company' && !$isProjectUser) {
                return redirect()->back()->with('error', __('Permission Denied.'));
            }
            ...
            if ($project) {
                $chartData = $this->getProjectChart(
                    ...
                    $user = Auth::user();
                    $user->update(['active_project' => $project->id]);
            }
            
            $invoices = \App\Models\PurchaseInvoice::with(['supplier','site','creator']) ->where('workspace_id', getActiveWorkSpace()) ->when(getActiveProject(), fn($q) => $q->where('site_id', getActiveProject())) ->orderBy('id', 'desc') ->limit(10) ->get();
            $activities = \App\Models\Activity::with(['creator','completeds']) ->where('workspace_id', getActiveWorkSpace()) ->where('site_id', getActiveProject()) ->orderBy('id', 'desc') ->limit(10) ->get();
           $manpowers = \App\Models\ManPowerMaster::with(['supplier','site','creator'])
                ->where('workspace_id', getActiveWorkSpace())
                ->where('site_id', getActiveProject())
                ->orderByDesc('work_date')
                ->limit(10)
                ->get();
            $consumptions = \App\Models\DailyConsumptionMaster::with(['site','creator'])
                ->where('site_id', getActiveProject())
                ->where('status', '!=', '1')
            ...
            $currentSiteId = getActiveProject();

            $transferredFrom = \App\Models\MaterialTransfer::with(['fromSite','toSite','creator'])
                ->where('from_site_id', $currentSiteId)
                ->where('status', '!=', '1')
                ->orderByDesc('record_date')
                ->limit(10)
                ->get();

            $transferredTo = \App\Models\MaterialTransfer::with(['fromSite','toSite','creator'])
            ...
            return view('taskly::projects.show', compact('project', 'daysleft', 'chartData','invoices','activities','manpowers','consumptions','transferredFrom','transferredTo'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function destroy($projectID)
    {
        if (!Auth::user()->isAbleTo('project delete')) {
            return response()->json(['status' => 0, 'message' => __('Permission denied.')], 400);
        }
        
        $objUser = Auth::user();
        $project = Project::find($projectID);

        if ($project->created_by == $objUser->id || $objUser->isAbleTo('project manage')) {
            
            // Check if material is linked in user_projects
            ...
            $existsInman_power = \DB::table('man_power_masters')
                ->where('site_id', $projectID)
                ->exists();
            ...
            $existsInDailyConsumption = \DB::table('daily_consumption_details')
             ...
             // Check if material is used in daily_consumption_details
            $existsInMaterial_transfers = \DB::table('material_transfers')
                ->where('site_id', $projectID)
                ->exists();
...
//        )->get();
        ...
        $workspace_users = User::where('created_by', '=', creatorId())->emp()->whereNOTIn(
            'id',
            function ($q) use ($project) {
                $q->select('user_id')->from('user_projects')->where('project_id', '=', $project->id);
            }
...
//             dd($registerUsers);
    ...
    public function gantt($projectID, $duration = 'Week')
    {
        if (\Auth::user()->isAbleTo('sub-task manage')) {
            ...
            if ($objUser->type == 'client' || $objUser->isAbleTo('project manage')) {
    ...
    public function taskBoard($projectID)
    {
        if (\Auth::user()->isAbleTo('task manage')) {
            ...
            $objUser = Auth::user();
            if (Auth::user()->hasRole('client')) {
                $project = Project::select('projects.*')->join('user_projects', 'projects.id', '=', 'user_projects.project_id')->where('projects.id', '=', $projectID)->first();
            } else {
                $project = Project::select('projects.*')->join('user_projects', 'projects.id', '=', 'user_projects.project_id')->where('user_projects.user_id', '=', $objUser->id)->where('projects.id', '=', $projectID)->first();
            }
            $stages = $statusClass = [];
            if ($project) {
                $stages = Stage::whereIn('id', function($query) {
    ...
    public function taskStore(Request $request, $projectID)
    {
        ...
        $request->validate(
            [
                ...
                'due_date' => 'required',
                'description' => 'required',
                'reference_file' => 'nullable|mimes:pdf,doc,docx,jpg,jpeg,png|max:10240',
            ]
        );
        $objUser          = Auth::user();
        $currentWorkspace = getActiveWorkSpace();
        ...
        if ($project) {
            ...
            if ($stage) {
                ...
                if ($request->hasFile('reference_file')) {
                    ...
                    $uploadPath = 'uploads/task_reference_files/';
                    $file->move($uploadPath, $filename);
                    $post['reference_file'] = $uploadPath . $filename;
                }
                ...
                // Send notification to assign_to users and created_by user
                $notificationService = app(\App\Services\NotificationService::class);
                $assignToUserIds = $request->assign_to;
                $notificationService->createTaskCreatedNotification(
                    $task->title,
                    $projectID,
                    $assignToUserIds,
                    Auth::user()->id,
    ...
    public function taskUpdate(Request $request, $projectID, $taskID)
    {
        ...
        if ($project) {
            ...
            if ($request->hasFile('reference_file')) {
                ...
                $task = Task::find($taskID);
                if ($task->reference_file && file_exists($task->reference_file)) {
                    unlink($task->reference_file);
                }
            }
            ...
            // Send notification to assign_to users and updated_by user
            $notificationService = app(\App\Services\NotificationService::class);
            $assignToUserIds = $request->assign_to;
            $notificationService->createTaskUpdatedNotification(
                $task->title,
                $projectID,
                $assignToUserIds,
                Auth::user()->id,
    ...
    public function commentStoreFile(Request $request, $projectID, $taskID, $clientID = '')
    {
        ...
        if ($upload['flag'] == 1) {
            ...
            // Send notification to assign_to users and uploaded_by user
            $notificationService = app(\App\Services\NotificationService::class);
            $task = Task::find($taskID);
            $assignToUserIds = explode(',', $task->assign_to);
            $uploadedByName = $TaskFile->user_type == 'Client' ? ($TaskFile->client ? $TaskFile->client->name : 'Unknown Client') : ($TaskFile->user ? $TaskFile->user->name : 'Unknown User');
    ...
    public function subTaskStore(Request $request, $projectID, $taskID, $clientID = '')
    {
        ...
        $task = Task::find($taskID);
        $assignToUserIds = explode(',', $task->assign_to);
        $createdByName = $subtask->user_type == 'Client' ? $subtask->client->name : $subtask->user->name;
        $notificationService->createSubtaskNotification(
            $task->title,
            $projectID,
            $taskID,
            $subtask->name,
    ...
    public function commentStore(Request $request, $projectID, $taskID, $clientID = '')
    {
        ...
        $notificationService = app(\App\Services\NotificationService::class);
        $assignToUserIds = explode(',', $task->assign_to);
        $createdByName = $comment->user_type == 'Client' ? $comment->client->name : $comment->user->name;
    ...
    public function sendTaskReminder(Request $request, $projectID, $taskID)
    {
        if (Auth::user()->isAbleTo('task manage')) {
            ...
            $assignedUserIds = array_filter($assignedUserIds, function($id) {
                return !empty($id) && is_numeric($id);
            });

            if (empty($assignedUserIds)) {
                return response()->json(['error' => __('No users assigned to this task.')], 400);
            }
            ...
            $notificationService->create(
                ...
                message: "{$currentUser->name} wants update about this task: {$task->title}",
                ...
                relatedId: $taskID,
                relatedType: 'Task',
                actionUrl: route('tasks.show', [$projectID, $taskID], false)
            );

            return response()->json([
                'success' => true,
                'message' => __('Reminder sent successfully to assigned users.')
            ]);
        } else {
    ...
    public function ProjectSharedLink($id = null, $lang = null)
    {
        ...
        if ($project) {
            ...
            if ($project) {
                $stages = Stage::orderBy('order')->distinct()->get();
                foreach ($stages as $status) {
                    $statusClass[] = 'task-list-' . str_replace(' ', '_', $status->id);
