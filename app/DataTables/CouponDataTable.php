...
class CouponDataTable extends DataTable
{
    ...
    {
        $rowColumn = ['type', 'discount', 'used_coupon', 'action', 'checkbox'];

        return (new EloquentDataTable($query))
            ->addIndexColumn()
            ->addColumn('checkbox', function (Coupon $coupon) {
                return '<input type="checkbox" class="' . $this->getCheckboxClass() . ' form-check-input" value="' . $coupon->id . '">';
            })
            ->editColumn('type',function(Coupon $coupon){
                return \App\Models\Coupon::$couponType[$coupon->type] ?? null;
            })
            ->editColumn('discount',function(Coupon $coupon){
                return  $coupon->discount .( $coupon->type == 'percentage' ? ' (%)' : '');
            })
            ->editColumn('used_coupon', function (Coupon $coupon) {
                return $coupon->used_coupon();
            })
    ...
    public function getColumns(): array
    {
        ...
        return [
            ...
            Column::make('limit_per_user')->title(__('Limit Per User')),
            Column::make('used_coupon')->title(__('Used'))->searchable(false)->orderable(false),
