...
{{ Form::open(['url' => 'announcement', 'method' => 'post', 'class' => 'needs-validation', 'novalidate']) }}
...
<div class="col-md-12">
    <div class="form-group">
        {{ Form::label('site_id', __('Project'), ['class' => 'form-label']) }} <x-required></x-required>
        <span id="site_id_span">
            <select class="form-control site_data choices" 
                    id="site_id" 
                    data-toggle="select2" 
                    required
                    name="site_id" 
    ...
    <p class="text-danger d-none" id="site_validation">{{ __('This field is required.') }}</p>
</div>
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                {{ Form::date('start_date', date('Y-m-d'), ['class' => 'form-control ', 'required' => 'required', 'placeholder' => __('Select Date'), 'min' => date('Y-m-d')]) }}
            </div>
        </div>
        <div class="col-md-6">
            <div class="form-group">
                {{ Form::label('end_date', __('Announcement end Date'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::date('end_date', date('Y-m-d'), ['class' => 'form-control ', 'required' => 'required', 'placeholder' => __('Select Date'), 'min' => date('Y-m-d')]) }}
            </div>
        </div>
        <div class="col-md-12">
            <div class="form-group">
