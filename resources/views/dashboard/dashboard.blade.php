...
@section('content')
@php
    $lastLedgerEntry = \App\Domain\Machinery\Models\MachineryLedger::where('is_reversal', false)
...
<!-- Last Ledger Entry Sanity Check -->
@if($lastLedgerEntry)
<div class="row mb-4">
    <div class="col-12">
        <div class="card bg-{{ $lastLedgerEntry->entry_direction === 'credit' ? 'success' : 'danger' }}-50 border border-{{ $lastLedgerEntry->entry_direction === 'credit' ? 'success' : 'danger' }}-200">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h6 class="mb-1 text-{{ $lastLedgerEntry->entry_direction === 'credit' ? 'success' : 'danger' }}">
                            <i class="ti ti-{{ $lastLedgerEntry->entry_direction === 'credit' ? 'arrow-up' : 'arrow-down' }}"></i>
                            Last Ledger Entry
                        </h6>
                        <div class="d-flex gap-4">
                            <span><strong>ID:</strong> #{{ $lastLedgerEntry->id }}</span>
                            <span><strong>Balance:</strong> ₹{{ number_format($lastLedgerEntry->running_balance, 2) }}</span>
                            <span><strong>Machine:</strong> {{ optional($lastLedgerEntry->machinery)->name ?? 'N/A' }}</span>
                            <span><strong>Time:</strong> {{ $lastLedgerEntry->created_at->diffForHumans() }}</span>
                        </div>
                    </div>
                    <a href="{{ route('ledger.index') }}" class="btn btn-sm btn-outline-{{ $lastLedgerEntry->entry_direction === 'credit' ? 'success' : 'danger' }}">
                        View Ledger
                    </a>
                </div>
            </div>
