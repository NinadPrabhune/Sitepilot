...
class PurchaseDataTable extends DataTable
{
    ...
    {
        $rowColumn = ['purchase_id','vender_id','category_id','purchase_date','status', 'checkbox'];
        $dataTable = (new EloquentDataTable($query))
        ->addIndexColumn()
        ->addColumn('checkbox', function (Purchase $purchase) {
            return '<input type="checkbox" class="' . $this->getCheckboxClass() . ' form-check-input" value="' . $purchase->id . '">';
        })
        ...
        ->addColumn('status', function ($purchase) {
            ...
            return '<span class="purchase_status badge ' . $class . ' status-btn p-2 px-3">' . Purchase::$statues[$purchase->status] . '</span>';
        })
    ...
    {
        $query = $model->where('purchases.created_by', creatorId())->where('purchases.workspace', getActiveWorkSpace());
    ...
    public function html(): HtmlBuilder
    {
        $dataTable = $this->builder()
            ...
            ->initComplete($this->getCheckboxInitScript() . "; " . 'function() {
                                var table = this;
