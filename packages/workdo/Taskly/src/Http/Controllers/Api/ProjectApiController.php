...
class ProjectApiController extends Controller
{
    ...
    public function projectCreateAndUpdate(Request $request)
    {
        ...
        try{
            ...
            if($request->project_id){
                ...
                // Ensure creator still has access
				...
				foreach ($userList as $email) {
                        ...
                        if ($registerUsers) {
                            ...
                            \Log::error('Project user assignment failed: User not found in workspace', [
                                'email' => $email,
                ...
                return response()->json(['status' => 1 , 'message' => 'Project Updated Successfully.'] , 200);
                    ...
                    // ALWAYS assign creator to project to prevent visibility issues
                    UserProject::firstOrCreate([
                        'user_id' => $objUser->id,
                    ...
                    foreach ($userList as $email) {
                        ...
                        if ($registerUsers) {
                            ...
                            // Log error for missing user - do NOT fail silently
                            \Log::error('Project user assignment failed: User not found in workspace', [
                                'email' => $email,
                                'workspace' => $currentWorkspace,
                                'project_id' => $objProject->id,
    ...
    public function destroyProject(Request $request)
    {
        try{
            ...
            if($project->created_by == $objUser->id || $objUser->isAbleTo('project delete'))
            {
