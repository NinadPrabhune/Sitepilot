...
class ModuleController extends Controller
{
    ...
    public function enable(Request $request)
    {
        ...
        if (!empty($module)) {
            ...
            if ($module->isEnabled()) {
                ...
                if (empty($addon)) {
                    // 🛡️ BLOCK MIGRATIONS FROM WEB REQUESTS
                    if (!app()->runningInConsole()) {
                        Log::critical('🚨 BLOCKED MIGRATION FROM WEB REQUEST', [
                            'module' => $request->name,
                            'user_id' => auth()->id(),
                            'ip' => request()->ip(),
                            'url' => request()->fullUrl(),
                        ...
                        $cliCommand = "php artisan module:setup {$request->name}";
                        return redirect()->back()->with('error',
                            "⚠️ Module database setup blocked for security.<br><br>" .
    ...
    public function install(Request $request)
    {
        ...
        if ($res === TRUE) {
            ...
            {
                // 🛡️ BLOCK SEEDERS FROM WEB REQUESTS
                if (!app()->runningInConsole()) {
                    Log::critical('🚨 BLOCKED SEEDER FROM WEB REQUEST', [
                        'module' => $fileName,
                        'user_id' => auth()->id(),
                        'ip' => request()->ip(),
                        'url' => request()->fullUrl(),
                    ...
                    $cliCommand = "php artisan db:seed --class=PackagesName";
                    return redirect()->back()->with('error',
                        "⚠️ Module seeder blocked for security.<br><br>" .
                        "Please run this command in your terminal:<br>" .
                        "<code>{$cliCommand}</code>"
                    );
                }
