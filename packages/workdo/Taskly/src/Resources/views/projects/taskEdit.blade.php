@if($project && $currentWorkspace && $task)
        {{ Form::model($task, array('route' => array('tasks.update',[$project->id,$task->id]), 'method' => 'Post','class'=>'needs-validation','novalidate','files'=>true)) }}
        @csrf
        <div class="modal-body">
            ...
            <div class="row">
                <div class="form-group col-md-8">
                    <label class="form-label">{{ __('Project')}}</label><x-required></x-required>
                ...
                <div class="form-group col-md-4">
                    ...
                    <select class="form-control form-control-light select2" name="stage_id" id="task-stage">
                        ...
                        @foreach($stages as $stage)
                            <option value="{{$stage->id}}" @if($task->status == $stage->id) selected @endif>{{$stage->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group col-md-12">
                    ...
                    <input type="file" class="form-control form-control-light" name="reference_file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png">
                    <small class="text-muted">{{ __('Accepted file types: pdf, doc, docx, jpg, jpeg, png (Max: 10MB)') }}</small>
                    @if($task->reference_file)
                        <div class="mt-2">
                            <a href="{{ asset($task->reference_file) }}" target="_blank" class="btn btn-sm btn-info">{{ __('View Current File') }}</a>
                        </div>
                    @endif
                </div>
