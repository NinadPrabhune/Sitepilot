...
class DesignationController extends Controller
{
    ...
    public function index()
    {
        if (Auth::user()->isAbleTo('designation manage')) {
            ...
            $designations = Designation::query()->with(['branch', 'department'])->get();
...
//            $designations = Designation::where('created_by', '=', creatorId())->where('workspace', getActiveWorkSpace())->with(['branch', 'department'])->get();
    ...
    public function create()
    {
        if (Auth::user()->isAbleTo('designation create')) {
            $branchs = Branch::query()->get()->pluck('name', 'id');
            $departments = Department::query()->get()->pluck('name', 'id');
    ...
    public function store(Request $request)
    {
        if (Auth::user()->isAbleTo('designation create')) {
            ...
            try {
                $branch = Department::query()->find($request->department_id)->branch->id;
            } catch (Exception $e) {
                $branch = null;
            }
