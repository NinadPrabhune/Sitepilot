    ...
    <div class="col-xl-3 col-md-6">
        <div class="card border-0 h-100" style="border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.05);">
            <div class="card-body p-4">
                ...
                <h2 class="fw-bold mb-1 counter" data-target="{{ $dashboardData['project']['budget'] }}" style="color: #0F172A;">
                    {{ currency_format_with_sym($dashboardData['project']['budget']) }}
        ...
        <a href="{{ route('spent.index') }}" class="text-decoration-none">
            <div class="card border-0 h-100" style="border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.05);">
                <div class="card-body p-4">
                    ...
                    <h2 class="fw-bold mb-1 counter" data-target="{{ $dashboardData['project']['total_spent'] }}" style="color: #0F172A;">
                        {{ currency_format_with_sym($dashboardData['project']['total_spent']) }}
                    </h2>
                    <p class="text-muted mb-0 small">{{ $spentPercent }}% of budget used</p>
                    <div class="mt-3">
                        <div class="progress" style="height: 6px; background: #E5E7EB; border-radius: 10px;">
                            <div class="progress-bar" role="progressbar" style="width: {{ min($spentPercent, 100) }}%; background: #F59E0B; border-radius: 10px;"></div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="col-xl-3 col-md-6">
        <div class="card border-0 h-100" style="border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.05);">
            <div class="card-body p-4">
                ...
                <h2 class="fw-bold mb-1 counter" data-target="{{ $dashboardData['project']['remaining_budget'] }}" style="color: #0F172A;">
                    {{ currency_format_with_sym($dashboardData['project']['remaining_budget']) }}
                </h2>
                <p class="text-muted mb-0 small">
                    @if($dashboardData['project']['remaining_budget'] < 0)
    ...
    <div class="col-xl-3 col-md-6">
        <div class="card border-0 h-100" style="border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.05);">
            <div class="card-body p-4">
                ...
                <h2 class="fw-bold mb-1" style="color: #0F172A;">{{ $dashboardData['project']['days_left'] }}</h2>
                <p class="text-muted mb-0 small">Days Remaining</p>
                <p class="text-muted mb-0 small mt-2">Due: {{ company_date_formate($dashboardData['project']['end_date']) }}</p>
            </div>
        </div>
    </div>
</div>
                    ...
                    <div class="col-md-2 text-center">
                        <div class="position-relative d-inline-block" style="width: 80px; height: 80px;">
                            <svg viewBox="0 0 100 100" style="width: 100%; height: 100%; transform: rotate(-90deg);">
                                ...
                                <circle cx="50" cy="50" r="38" fill="none" stroke="{{ $healthScore >= 70 ? '#22C55E' : ($healthScore >= 40 ? '#F59E0B' : '#EF4444') }}" stroke-width="8"
                                    stroke-dasharray="{{ $healthScore * 2.38 }} 238"
                                    stroke-linecap="round"/>
                            </svg>
                            <div class="position-absolute top-50 start-50 translate-middle text-center">
                                <h5 class="mb-0 fw-bold" style="color: #0F172A;">{{ $healthScore }}</h5>
                        ...
                        <div class="row g-3">
                            <div class="col-md-3 col-6">
                                <div class="text-center p-3 rounded-3" style="background: #F8FAFC; border: 1px solid #E5E7EB;">
                                    <i class="ti ti-shield-check fs-4 mb-1" style="color: {{ $riskLevel == 'low' ? '#22C55E' : ($riskLevel == 'medium' ? '#F59E0B' : '#EF4444') }};"></i>
                                    <h6 class="mb-1 fw-semibold" style="color: #0F172A;">{{ ucfirst($riskLevel) }} Risk</h6>
                                    <small class="text-muted">Risk Level</small>
                                </div>
                            </div>
                            <div class="col-md-3 col-6">
                                <div class="text-center p-3 rounded-3" style="background: #F8FAFC; border: 1px solid #E5E7EB;">
                                    <i class="ti ti-calendar-check fs-4 mb-1" style="color: {{ $scheduleStatus == 'on_track' ? '#22C55E' : ($scheduleStatus == 'ahead' ? '#4F46E5' : '#EF4444') }};"></i>
                                    <h6 class="mb-1 fw-semibold" style="color: #0F172A;">{{ str_replace('_', ' ', ucwords($scheduleStatus, '_')) }}</h6>
                                    <small class="text-muted">Schedule</small>
                                </div>
                            </div>
                            <div class="col-md-3 col-6">
                                <div class="text-center p-3 rounded-3" style="background: #F8FAFC; border: 1px solid #E5E7EB;">
                                    <i class="ti ti-wallet fs-4 mb-1" style="color: {{ $budgetStatus == 'within_budget' ? '#22C55E' : ($budgetStatus == 'near_limit' ? '#F59E0B' : '#EF4444') }};"></i>
                                    <h6 class="mb-1 fw-semibold" style="color: #0F172A;">{{ str_replace('_', ' ', ucwords($budgetStatus, '_')) }}</h6>
                            ...
                            <div class="col-md-3 col-6">
                                <div class="text-center p-3 rounded-3" style="background: #F8FAFC; border: 1px solid #E5E7EB;">
                                    ...
                                    <h6 class="mb-1 fw-semibold" style="color: #0F172A;">{{ $activityProgress }}%</h6>
...
<div class="row g-4 mb-4">
    ...
    <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
        <a href="{{ route('indent.index', ['site_id' => $project->id]) }}" class="text-decoration-none">
            <div class="procurement-card text-center">
                ...
                <div class="mt-3">
                    ...
                    <h5 class="fw-bold">{{ $dashboardData['procurement']['indent']['total_indent'] ?? 0 }}</h5>
                    <p class="mb-1 text-muted small">Today</p>
                    <h6 class="fw-semibold text-primary">{{ $dashboardData['procurement']['indent']['today_indent'] ?? 0 }}</h6>
                </div>
            </div>
        </a>
    </div>
    ...
    <!-- Purchase Order -->
    <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
        <a href="{{ route('purchase-order.index', ['site_id' => $project->id]) }}" class="text-decoration-none">
            <div class="procurement-card text-center">
                <i class="ti ti-shopping-cart fs-32" style="color: #F59E0B;"></i>
                <h6 class="mt-2 fw-semibold">Purchase Order</h6>
                <div class="mt-3">
                    <p class="mb-1 text-muted small">All</p>
                    <h5 class="fw-bold">{{ $dashboardData['procurement']['po']['total_po'] ?? 0 }}</h5>
                    <p class="mb-1 text-muted small">Today</p>
                    <h6 class="fw-semibold text-primary">{{ $dashboardData['procurement']['po']['today_po'] ?? 0 }}</h6>
                </div>
            </div>
        </a>
    </div>
    ...
    <!-- GRN -->
    <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
        <a href="{{ route('grn.index', ['site_id' => $project->id]) }}" class="text-decoration-none">
            <div class="procurement-card text-center">
                <i class="ti ti-truck-delivery fs-32" style="color: #22C55E;"></i>
                <h6 class="mt-2 fw-semibold">GRN</h6>
                <div class="mt-3">
                    <p class="mb-1 text-muted small">All</p>
                    <h5 class="fw-bold">{{ $dashboardData['procurement']['grn']['total_grn'] ?? 0 }}</h5>
                    <p class="mb-1 text-muted small">Today</p>
                    <h6 class="fw-semibold text-primary">{{ $dashboardData['procurement']['grn']['today_grn'] ?? 0 }}</h6>
                </div>
            </div>
        </a>
    </div>
    ...
    <!-- Invoice -->
    <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
        <a href="{{ route('purchase-invoice.index', ['site_id' => $project->id]) }}" class="text-decoration-none">
            <div class="procurement-card text-center">
                <i class="ti ti-receipt fs-32" style="color: #EF4444;"></i>
                <h6 class="mt-2 fw-semibold">Invoice</h6>
                <div class="mt-3">
                    <p class="mb-1 text-muted small">All</p>
                    <h5 class="fw-bold">{{ $dashboardData['procurement']['invoice']['total_invoice'] ?? 0 }}</h5>
                    <p class="mb-1 text-muted small">Today</p>
                    <h6 class="fw-semibold text-primary">{{ $dashboardData['procurement']['invoice']['today_invoice'] ?? 0 }}</h6>
                </div>
            </div>
        </a>
    </div>
                ...
                <div class="row align-items-center">
                    <div class="col-md-8">
                        ...
                        <div class="d-flex gap-3">
                            <span class="badge bg-primary bg-opacity-10 text-primary">Activity: {{ $activityProgress }}%</span>
                            <span class="badge bg-info bg-opacity-10 text-info">Time: {{ $timeProgress }}%</span>
                            <span class="badge bg-warning bg-opacity-10 text-warning">Budget: {{ $spentPercent }}%</span>
                        </div>
                    </div>
                    <div class="col-md-4 text-md-end mt-3 mt-md-0">
                        <div class="position-relative d-inline-block" style="width: 100px; height: 100px;">
                            <svg viewBox="0 0 100 100" style="width: 100%; height: 100%; transform: rotate(-90deg);">
                                ...
                                <circle cx="50" cy="50" r="42" fill="none" stroke="{{ $overallProgress >= 70 ? '#22C55E' : ($overallProgress >= 40 ? '#F59E0B' : '#EF4444') }}" stroke-width="10"
                                    stroke-dasharray="{{ $overallProgress * 2.64 }} 264"
                                    stroke-linecap="round"/>
                            </svg>
                            <div class="position-absolute top-50 start-50 translate-middle text-center">
                                <h4 class="mb-0 fw-bold" style="color: #0F172A;">{{ $overallProgress }}%</h4>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="mt-4">
                    <div class="progress" style="height: 12px; background: #E5E7EB; border-radius: 10px;">
                        <div class="progress-bar" role="progressbar" 
                            style="width: {{ $overallProgress }}%; background: linear-gradient(90deg, {{ $overallProgress >= 70 ? '#22C55E' : ($overallProgress >= 40 ? '#F59E0B' : '#EF4444') }}, {{ $overallProgress >= 70 ? '#16A34A' : ($overallProgress >= 40 ? '#D97706' : '#DC2626') }}); border-radius: 10px;">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body p-4">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        ...
                        <p class="text-muted mb-0">{{ $dashboardData['activities']['completed_activities'] }} of {{ $dashboardData['activities']['total_activities'] }} activities completed ({{ $activityProgress }}%)</p>
                    </div>
                    <div class="col-md-4 text-md-end mt-3 mt-md-0">
                        <div class="position-relative d-inline-block" style="width: 80px; height: 80px;">
                            <svg viewBox="0 0 100 100" style="width: 100%; height: 100%; transform: rotate(-90deg);">
                                ...
                                <circle cx="50" cy="50" r="38" fill="none" stroke="#4F46E5" stroke-width="8"
                                    stroke-dasharray="{{ $activityProgress * 2.38 }} 238"
                                    stroke-linecap="round"/>
                            </svg>
                            <div class="position-absolute top-50 start-50 translate-middle text-center">
                                <h5 class="mb-0 fw-bold" style="color: #0F172A;">{{ $activityProgress }}%</h5>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="mt-4">
                    <div class="progress" style="height: 10px; background: #E5E7EB; border-radius: 10px;">
                        <div class="progress-bar" role="progressbar" 
                            style="width: {{ $activityProgress }}%; background: linear-gradient(90deg, #4F46E5, #6366F1); border-radius: 10px;">
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <a href="{{ route('indent.index', ['site_id' => $project->id]) }}" class="text-decoration-none">
                            <div class="pipeline-card p-3 rounded-3 h-100">
                                ...
                                <div class="d-flex justify-content-between mb-1">
                                    ...
                                    <span class="fw-bold" style="color: #F59E0B;">{{ $dashboardData['procurement']['indent']['open_indent'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="text-muted small">Closed</span>
                                    <span class="fw-bold" style="color: #22C55E;">{{ $dashboardData['procurement']['indent']['closed_indent'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span class="text-muted small">Today</span>
                                    <span class="fw-bold" style="color: #4F46E5;">{{ $dashboardData['procurement']['indent']['today_indent'] }}</span>
                                </div>
                                <hr class="my-2" style="border-color: #E5E7EB;">
                                <div class="text-center">
                                    <span class="badge rounded-pill px-3 py-2" style="background: #4F46E5; color: white;">{{ $dashboardData['procurement']['indent']['total_indent'] }} Total</span>
                                </div>
                            </div>
                        </a>
                    </div>
                        ...
                        <a href="{{ route('purchase-order.index', ['site_id' => $project->id]) }}" class="text-decoration-none">
                            <div class="pipeline-card p-3 rounded-3 h-100">
                                ...
                                <div class="d-flex justify-content-between mb-1">
                                    ...
                                    <span class="fw-bold" style="color: #64748B;">{{ $dashboardData['procurement']['po']['draft_po'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="text-muted small">Pending</span>
                                    <span class="fw-bold" style="color: #F59E0B;">{{ $dashboardData['procurement']['po']['pending_po'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="text-muted small">Approved</span>
                                    <span class="fw-bold" style="color: #22C55E;">{{ $dashboardData['procurement']['po']['approved_po'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="text-muted small">Partial Rec.</span>
                                    <span class="fw-bold" style="color: #8B5CF6;">{{ $dashboardData['procurement']['po']['partial_received_po'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="text-muted small">Completed</span>
                                    <span class="fw-bold" style="color: #4F46E5;">{{ $dashboardData['procurement']['po']['completed_po'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="text-muted small">Rejected</span>
                                    <span class="fw-bold" style="color: #EF4444;">{{ $dashboardData['procurement']['po']['rejected_po'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="text-muted small">Flagged</span>
                                    <span class="fw-bold" style="color: #F97316;">{{ $dashboardData['procurement']['po']['flagged_po'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span class="text-muted small">Short Closed</span>
                                    <span class="fw-bold" style="color: #6B7280;">{{ $dashboardData['procurement']['po']['short_closed_po'] }}</span>
                                </div>
                                <hr class="my-2" style="border-color: #E5E7EB;">
                                <div class="text-center">
                                    <span class="badge rounded-pill px-3 py-2" style="background: #F59E0B; color: white;">{{ $dashboardData['procurement']['po']['total_po'] }} Total</span>
                                </div>
                            </div>
                        </a>
                    </div>
                    ...
                    <div class="col-xl-2 col-md-6">
                        <a href="{{ route('grn.index', ['site_id' => $project->id]) }}" class="text-decoration-none">
                            <div class="pipeline-card p-3 rounded-3 h-100">
                                ...
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="text-muted small">Pending</span>
                                    <span class="fw-bold" style="color: #F59E0B;">{{ $dashboardData['procurement']['grn']['pending_grn'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="text-muted small">Partial</span>
                                    <span class="fw-bold" style="color: #8B5CF6;">{{ $dashboardData['procurement']['grn']['partial_grn'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span class="text-muted small">Completed</span>
                                    <span class="fw-bold" style="color: #22C55E;">{{ $dashboardData['procurement']['grn']['completed_grn'] }}</span>
                                </div>
                                <hr class="my-2" style="border-color: #E5E7EB;">
                                <div class="text-center">
                                    <span class="badge rounded-pill px-3 py-2" style="background: #22C55E; color: white;">{{ $dashboardData['procurement']['grn']['total_grn'] }} Total</span>
                                </div>
                            </div>
                        </a>
                    </div>
                        ...
                        <a href="{{ route('purchase-invoice.index', ['site_id' => $project->id]) }}" class="text-decoration-none">
                            <div class="pipeline-card p-3 rounded-3 h-100">
                                ...
                                <div class="d-flex justify-content-between mb-1">
                                    ...
                                    <span class="fw-bold" style="color: #22C55E;">{{ $dashboardData['procurement']['invoice']['paid_invoice'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="text-muted small">Partially Paid</span>
                                    <span class="fw-bold" style="color: #8B5CF6;">{{ $dashboardData['procurement']['invoice']['partially_paid_invoice'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="text-muted small">Unpaid</span>
                                    <span class="fw-bold" style="color: #EF4444;">{{ $dashboardData['procurement']['invoice']['unpaid_invoice'] }}</span>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span class="text-muted small">Overdue</span>
                                    <span class="fw-bold" style="color: #F59E0B;">{{ $dashboardData['procurement']['invoice']['overdue_invoice'] }}</span>
                                </div>
                                <hr class="my-2" style="border-color: #E5E7EB;">
                                <div class="text-center">
                                    <span class="badge rounded-pill px-3 py-2" style="background: #EF4444; color: white;">{{ $dashboardData['procurement']['invoice']['total_invoice'] }} Total</span>
                                </div>
                                <div class="text-center mt-2">
                                    <small class="text-muted">{{ currency_format_with_sym($dashboardData['procurement']['invoice']['total_invoice_amount']) }}</small>
                                </div>
                            </div>
                        </a>
                    </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-4">
                        <div class="p-3 rounded-3 text-center" style="background: #F8FAFC; border: 1px solid #E5E7EB;">
                            ...
                            <h4 class="mb-0 fw-bold counter" data-target="{{ $dashboardData['manpower']['total_workers_today'] }}" style="color: #0F172A;">{{ $dashboardData['manpower']['total_workers_today'] }}</h4>
                    ...
                    <div class="col-4">
                        <div class="p-3 rounded-3 text-center" style="background: #F8FAFC; border: 1px solid #E5E7EB;">
                            ...
                            <h4 class="mb-0 fw-bold" style="color: #0F172A;">{{ $dashboardData['manpower']['manpower_records_today'] }}</h4>
                    ...
                    <div class="col-4">
                        <div class="p-3 rounded-3 text-center" style="background: #F8FAFC; border: 1px solid #E5E7EB;">
                            ...
                            <h4 class="mb-0 fw-bold" style="color: #0F172A;">{{ $dashboardData['manpower']['total_contractors'] }}</h4>
            ...
            <div class="card-body px-4 pb-4">
                <div class="text-center mb-3">
                    <span class="badge rounded-pill px-4 py-2" style="background: #4F46E5; color: white; font-size: 14px;">
                        Today: {{ currency_format_with_sym($dashboardData['consumption']['total_consumption_today']) }}
                    </span>
                </div>
                ...
                <div class="row g-3">
                    <div class="col-6">
                        <div class="p-4 rounded-3 text-center" style="background: #FEF3C7; border: 1px solid #FCD34D;">
                            ...
                            <h3 class="mb-0 fw-bold counter" data-target="{{ $dashboardData['transfers']['materials_transferred_out'] }}" style="color: #0F172A;">{{ $dashboardData['transfers']['materials_transferred_out'] }}</h3>
                    ...
                    <div class="col-6">
                        <div class="p-4 rounded-3 text-center" style="background: #DBEAFE; border: 1px solid #93C5FD;">
                            ...
                            <h3 class="mb-0 fw-bold counter" data-target="{{ $dashboardData['transfers']['materials_transferred_in'] }}" style="color: #0F172A;">{{ $dashboardData['transfers']['materials_transferred_in'] }}</h3>
        ...
        <div class="card border-0 h-100" style="border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.05);">
            ...
            <div class="card-body px-4 pb-4">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <div class="p-3 rounded-3 d-flex align-items-center" style="background: {{ $alert['type'] == 'danger' ? '#FEF2F2' : ($alert['type'] == 'warning' ? '#FFFBEB' : '#EFF6FF') }}; border: 1px solid {{ $alert['type'] == 'danger' ? '#FECACA' : ($alert['type'] == 'warning' ? '#FDE68A' : '#BFDBFE') }};">
                            <div class="p-2 rounded me-3" style="background: white;">
                                <i class="ti {{ $alert['icon'] }} fs-5" style="color: {{ $alert['type'] == 'danger' ? '#EF4444' : ($alert['type'] == 'warning' ? '#F59E0B' : '#3B82F6') }};"></i>
                            </div>
                            <div>
                                <h6 class="mb-0 fw-semibold" style="color: #0F172A;">{{ $alert['title'] }}</h6>
                        ...
                        <div class="col-md-6 col-lg-4 mb-3">
                            <div class="d-flex align-items-start p-3 rounded-3" style="background: #F8FAFC; border: 1px solid #E5E7EB;">
                                ...
                                <div class="flex-grow-1">
                                    @if(($item['type'] ?? '') === 'invoice')
                                    <h6 class="mb-0 fw-semibold" style="color: #0F172A;">{{ $item['invoice_number'] ?? '' }}</h6>
                                    <span class="badge bg-{{ $item['payment_status'] === 'paid' ? 'success' : 'secondary' }}">{{ ucfirst($item['payment_status'] ?? '') }}</span>
                                    @elseif(($item['type'] ?? '') === 'activity')
                                    <h6 class="mb-0 fw-semibold" style="color: #0F172A;">{{ $item['title'] ?? '' }}</h6>
                                    <span class="badge bg-{{ $item['status'] === 'completed' ? 'success' : 'warning' }}">{{ ucfirst($item['status'] ?? '') }}</span>
                                    @else
                                    <h6 class="mb-0 fw-semibold" style="color: #0F172A;">{{ $item['total_count'] ?? 0 }} Workers</h6>
                                    @endif
                                    <small class="text-muted d-block">{{ isset($item['created_at']) ? \Carbon\Carbon::parse($item['created_at'])->diffForHumans() : '' }}</small>
                                </div>
                            </div>
                        </div>
                    @empty
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.counter').forEach(function(el) {
        ...
        const prefix = match ? match[1] : '{{ company_setting("defult_currancy") }} ';
    ...
    const monthlyData = @json($dashboardData['charts']['monthly_spending']);
    const monthlyBudget = {{ $dashboardData['charts']['monthly_budget'] }};
    ...
    // Budget Donut Chart
    const budgetSpent = {{ $dashboardData['project']['total_spent'] }};
    const budgetRemain = Math.max(0, {{ $dashboardData['project']['remaining_budget'] }});
    ...
    const manpowerData = @json($dashboardData['manpower']['manpower_trend']);
    ...
    const consumptionData = @json($dashboardData['consumption']['top_consumed_materials']);
